...
def generate_calendar(year: int, month: int, user_id: int) -> str:
    ...
    # Дни недели - используем короткие названия
    weekdays = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс']
    # Фиксированная ширина для каждого дня недели (3 символа)
    ...
    # Создаем календарь построчно с фиксированной шириной
    calendar_lines = []
    current_line = []
    
    # Отступ для первого дня (каждый день занимает 3 символа)
    for _ in range(first_weekday):
        current_line.append('   ')  # 3 пробела для пустого дня
    ...
    for day in range(1, days_in_month + 1):
        if day in dates_data:
            # День с вышивкой - используем символ * и число
            count = dates_data[day]
            total_days += 1
            total_stitches += count
            # Фиксированная ширина: символ * + число (всего 3 символа)
            day_str = f'*{day:2d}'
        else:
            # День без вышивки - только число с отступом (3 символа)
            day_str = f'{day:2d} '
        ...
        if len(current_line) == 7:
            # Соединяем с двумя пробелами между днями
    ...
    if total_days > 0:
        avg = total_stitches / total_days  # Используем обычное деление для поддержки дробных чисел
